...
The browser never talks to PostgreSQL. Vite (`:5173`) calls Spring Boot (`:8080`) with `fetch`;
CORS is allowed only for that origin. The controller maps the HTTP contract onto a use case,
the use case talks to the domain, and only the persistence adapter touches JPA.

```mermaid
sequenceDiagram
    participant UI as Vite SPA :5173
    participant API as Controller :8080
    participant UC as Use case
    participant Dom as Domain
    participant DB as PostgreSQL :5432

    UI->>API: POST /api/v1/training-sessions
    API->>UC: RecordTrainingSessionUseCase
    UC->>Dom: TrainingSession + RepsCount
    alt business rule broken
        Dom-->>API: domain exception
        API-->>UI: ErrorResponse JSON
    else valid set
        UC->>DB: SessionRepository.save
        DB-->>UI: 201 + session JSON
        UI->>API: GET /api/v1/leaderboards/PULL_UP
        API-->>UI: ranking rebuilt from stored sets
    end
```

Register, update profile and friend-request flows follow the same path: HTTP in, use case,
domain invariants, adapter, PostgreSQL, JSON out.
